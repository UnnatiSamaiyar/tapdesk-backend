...
router.post("/thread",verifyToken,upload.array('attached'), async (req, res) => {
  try {
...
router.get("/threadByTask/:id",verifyToken, async (req, res) => {
  try {
    
    const {id} = req.params;
    const threads = await Task.find({task:id}).populate('createdBy');
...
router.put('/threadDeleteAttachment/:threadId',verifyTokenAndRoles(["admin", "Account Manager"]), async (req, res) => {
